    ...
    <div className='bg-setBlue-100 flex flex-col items-center p-[20px] rounded relative f-full'>
      ...
      <div className='flex flex-col lg:flex-row justify-around w-full lg:flex-nowrap mt-3 mb-3'>
        ...
        <div className='flex flex-col items-center w-full mb-5'>
          <a href={route + products[0].id} className='h-[20rem] w-[20rem] lg:h-[540px] lg:w-[540px] overflow-hidden rounded bg-black first-card'>
            <div className='w-full h-full bg-cover bg-center bg-no-repeat' style={{ backgroundImage: `url(`+products[0].picture+`)` }} ></div>
          </a>
          <h1 className='hidden lg:block font-bold font-source uppercase text-setGray-100 text-3xl text-center mt-3'>{products[0].name.replace(/[_]/gi, ' ')}</h1>
        </div>

        <div className='w-full flex flex-wrap justify-evenly'>
        {products.map((product, index) => {
            return index > 0 ? <a key={index} href={route + product.id} className='h-[20rem] w-[20rem] lg:h-[250px] lg:w-[250px] overflow-hidden rounded bg-black other-card relative mb-5 lg:mb-0'>
